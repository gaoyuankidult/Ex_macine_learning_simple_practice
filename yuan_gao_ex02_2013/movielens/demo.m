...
%
  ...
  if (task==2),
    ...
    for (i=1:length(ratings))
        % get item id
        ind = ratings(i,2);
        % sum according to rating
        sums(ind) = sums(ind)+ratings(i,3);
    end
    % get for how many times each movie has been rated
    cnt = hist(ratings(:,2),max(itemids));
    % get average rating of each item
    avg = sums./cnt;
    %?
    ...
    % only consider the situation when there is enough rating
    cnt = cnt( suff );
    avg = avg( suff );
    
    % sort arg in revirse order and return the ranking in si
    [dummy,si] = sort(-avg);
